...
|---|---|
| UserMessage | 用户人工输入 |
| EventMessage | 定时触发器注入的事件消息 |
| AssistantMessage | LLM 最终回复 |
| ReasoningBlock | Provider 返回的可见推理 / 推理摘要 |
| ToolCallCard | 工具调用卡片 |
| ToolResultCard | 工具结果卡片 |
| OrderResultCard | 买入 / 卖出 / 撤单结果 |
...
type ChatSession = {
  ...
  model: string;
  skillId: string;
  status: "idle" | "running" | "queued" | "error" | "archived";
  lastRunAt?: string;
  hasTriggers: boolean;
};
```
...
type WsEvent =
  ...
  | { type: "llm.delta"; payload: Delta }
  | { type: "reasoning.delta"; payload: Delta }
  | { type: "tool.started"; payload: ToolCall }
  | { type: "tool.finished"; payload: ToolResult }
  | { type: "order.created"; payload: Order }
  | { type: "trade.created"; payload: Trade }
  | { type: "portfolio.updated"; payload: PortfolioState }
  | { type: "run.finished"; payload: Run }
