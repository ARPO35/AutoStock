...
资产变化折线图       ← 未实现（占位 EmptyState）
数字信息面板         ← 已实现（部分指标）
持仓股票列表，可展开 ← 未实现（占位 EmptyState）
交易记录折叠栏       ← 未实现（占位 EmptyState）
```
...
**实现状态：7 个子页面中 3 个已实现，4 个未实现**

| 子页面 | 路由 | 状态 |
|---|---|---|
| 总览 | /view (默认) | 已实现（账号卡片 + 统计指标网格） |
| 账号详情 | — | 已实现（accounts + sessions 表格） |
| 股票信息 | — | 已实现（查询表单 + 行情+图表） |
| 交易历史 | — | 未实现（占位） |
| 资产曲线 | — | 未实现（占位） |
| 决策日志 | — | 未实现（占位） |
...
订单创建 (order.created)                      ← 未实现
成交创建 (trade.created)                      ← 未实现
账户更新 (portfolio.updated)                  ← 未实现
资产曲线更新                                  ← 未实现
错误 (error)                                  ← 已实现
```
